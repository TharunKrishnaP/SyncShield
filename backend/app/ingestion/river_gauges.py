...
def enrich_with_live_flow(reading: Dict[str, Any], live: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # The base reading's own provenance label is preserved: a base that came
    # from the CWC published-threshold fallback keeps CWC_PUBLISHED; only a
    # base generated by the scenario player keeps SCENARIO_MODEL. The blended
    # record's data_source always names both the real flow feed and the base.
    base_src = reading.get("water_level_source") or "CWC_PUBLISHED"
    reading.update({
        ...
        "data_source": "OPEN_METEO_GLOFAS+" + ("SCENARIO" if base_src == "SCENARIO_MODEL" else "CWC_PUBLISHED"),
        "water_level_source": base_src,
